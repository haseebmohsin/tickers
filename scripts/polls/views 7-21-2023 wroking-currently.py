...
import time
from .models import Ticker_Extraction_Model,Character_Comparison,FrameSaver
...
def geo_ticker(request):
    ...
    characters=streamer(source="https://www.youtube.com/watch?v=C6Se87yOvrk",y1=297,y2=335,x1=89,x2=550,folder_save='Geo_Ticker')            
...
def dunya_ticker(request):
    ...
    geo_ticker_remove=[]
    if len(geo_ticker_remove)>=100:
      file_remover(folder_name='headline_checker',extension_name='Geo',appended_file_name=geo_ticker_remove)
   
    characters,image=streamer(source="https://www.youtube.com/watch?v=O3DPVlynUM0",y1=262,y2=287,x1=162,x2=401,folder_save='Dunya_Ticker')            
...
def express_ticker(request):
    ...
    Getting_Foreign_Object = FrameSaver.objects.select_related('Ticker_Model')
    print('Getting_Foreign_Object',Getting_Foreign_Object)
