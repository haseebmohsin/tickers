...
def index(request):
    if request.method=='POST':
            ...
            return render (request,'index.html',{'result':result[:10],'channel_name':channel_name_search,'form':form})
    else:
...
def notifications(request):
    ...
    if len(get_words>=1):
        ...
        for words in range(len(words_list)):
            print('words_list[words]',words_list[words])
            results = Ticker_Extraction_Model.objects.filter(text_ocr__icontains=words_list[words],date=date).values()
        # data = list(results)
            data = list(results)

                # Store the data for the current word in the results_dict
            results_dict[words_list[words]] = data
        ...
        return JsonResponse({'success':'success','notification_data':results_dict,'notification_count':notifications_count,'data_length':length},safe=False)
    else:
        return JsonResponse({'error':'error'})
