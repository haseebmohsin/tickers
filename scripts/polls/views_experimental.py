...
from .models import Ticker_Extraction_Model,words_to_be_searched
...
def index(request):
    if request.method=='POST':
            ...
            checking=words_to_be_searched.objects.filter(word=wordnotification).exists()
            if checking:
                pass
            else:
                words_to_be_searched.objects.create(word=wordnotification)
...
length_all_objects=0
def notifications(request):
    global length_all_objects
    ...
    date=datetime.datetime.now().date()
        ...
        # data=json.loads(request.body)
    get_words=words_to_be_searched.objects.all()
    ...
    if len(get_words)>=1:
        results_dict = {}
        official_result=[]
        print('counter_for_notification',counter_for_notifications)
      
        # if len(get_words)>1:
        for i in range(len(get_words)):
            # print('words_list[words]',get_words[words])
            if counter_for_notifications>=1:
                results = Ticker_Extraction_Model.objects.filter(text_ocr__icontains=get_words[i].word,date=date).order_by('-id')[words_to_get[i]:].values()
                print('lengthifstatement',len(results))
                # length_all_objects=len(results)
                # print('resultsif',len(results))
                data = list(results)
                results_dict[get_words[i].word] = data

            elif counter_for_notifications==0:
                    results = Ticker_Extraction_Model.objects.filter(text_ocr__icontains=get_words[i].word,date=date).order_by('-id').values()
                    data = list(results)
            
                    results_dict[get_words[i].word] = data

            # elif counter_for_notifications>=1:
            #     results = Ticker_Extraction_Model.objects.filter(text_ocr__icontains=i.word,date=date).order_by('-id').values()
            #     print('resultselif',len(results))
            #     # length_overall=length_all_objects+len(results)
            #     # length_all=length_overall
            #     data = list(results)
        ...
        if counter_for_notifications==0:
            for key,val in results_dict.items():
                length+=len(val)
                words_to_get.append(len(val))

            new_notifications=length

            
                # length=length_all_objects
            # length=length+le
            ...
            length=length_all_objects+length

        print('words_to_get',words_to_get)
        print('length_all_objects',length_all_objects)
        ...
        #         official_result.append(first_data_for_word)
        ...
        return JsonResponse({'new_notifications':new_notifications,'notification_increment':notification_increment,'get_words_dict':list(get_words_dict),'success':'success','notification_data':results_dict,'data_length':length},safe=False)
    else:
        return JsonResponse({'error':'error'})

def count_incrementer(request):
    global counter_for_notifications
    global length_all_objects
    if request.method=="POST":
        data=json.loads(request.body)
        counted_value=data['countedValue']
        print('valueLoading',counted_value)
        length_all_objects=data['oldNotificationsViewButton']
        # print('data_total',data['oldNotificationsViewButton'])
